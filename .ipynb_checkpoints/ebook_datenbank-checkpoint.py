...
class DATABASE:
    ...
    def update_title(self, book_id, new_title):
        ...
        sql_command += " WHERE id = {};".format(book_id)
        if self.__verbose: print(sql_command)
        crsr.execute(sql_command)
        table = crsr.fetchall()
        connection.commit()
    ...
    def update_year(self, book_id, new_year):
        connection = sqlite3.connect(self.__name_db)
        crsr = connection.cursor()
        
        sql_command = "UPDATE books SET year = '{}' ".format(new_year)
        sql_command += " WHERE id = {};".format(book_id)
        if self.__verbose: print(sql_command)
        crsr.execute(sql_command)
        table = crsr.fetchall()
    ...
    def select_path(self, book_id):
        connection = sqlite3.connect(self.__name_db)
        crsr = connection.cursor()
        
        sql_command = "SELECT path FROM books WHERE id = {};".format(book_id)
        if self.__verbose: print(sql_command)
        crsr.execute(sql_command)
        table = crsr.fetchall()
